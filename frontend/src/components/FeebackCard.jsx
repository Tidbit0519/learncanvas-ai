...
const FeedbackCard = ({ assignment, submission }) => {
	...
	const { loading, error, feedback, getFeedback, setFeedback } = useFeedbackApi();
	const prompt =
    ...
    console.log("openAPIFlag", openAPIFlag);
	...
	return (
		<div className="bg-slate-700 rounded-lg p-4 flex flex-col gap-2">
            ...
            <div>
                <button
                    className={`p-3 py-2 ml-auto rounded-xl text-slate-100 hover:bg-violet-800 tracking-wider max-w-[200px] text-sm min-w-fit ${
                        feedback !== ""
                            ? "bg-violet-700"
                            : "bg-slate-500"
                    }`}
